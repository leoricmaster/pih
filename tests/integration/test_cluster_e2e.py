...
# 尊重 .env/.env.defaults 覆盖；剥 +psycopg driver 前缀（psycopg.connect 需裸 DSN）
...
def _ok_pred(subject: str = "三一", event_type: str = "新品发布") -> dict:
    """能过真实领域包 validate_pred 的输出。"""
    return {
        "主体": subject,
        "事件类型": event_type,
        "事实描述": f"{subject}发布新品",
...
def _seed_intel(
    ...
    *,
    ...
    """直接 INSERT 一条 extracted 状态 intel_item 供聚类测试。

    跳过 collect+process 流程，单测聚类逻辑本身。
    """
    with psycopg.connect(PG_DSN) as conn, conn.cursor() as cur:
        cur.execute(
...
def _cluster_one(intel_id: int) -> int | None:
    """对单条情报跑聚类（真实 EventService）。"""
    pool = get_pool()
    try:
...
class TestAC1NewEventCreated:
    def test_single_extracted_creates_pending_event(self):
        ...
        intel_id = _seed_intel(
            "sany", "三一", "新品发布", datetime(2026, 8, 27, 10, 0, 0)
        )
        event_id = _cluster_one(intel_id)
...
class TestAC2SecondIndependentSourceAdvances:
    def test_second_source_triggers_single_source(self):
        """AC2：同主体+同事件类型+时间窗内、第二独立信源 → pending→single_source + ready_for_manual。"""
        t = datetime(2026, 8, 27, 10, 0, 0)
        id1 = _seed_intel("sany", "三一", "新品发布", t)
        _cluster_one(id1)

        # 第二条：异源 ccma，3 天后（时间窗内）
        id2 = _seed_intel("ccma", "三一", "新品发布", t + timedelta(days=3))
        event_id2 = _cluster_one(id2)
...
class TestAC3SameSourceDoesNotAdvance:
    def test_same_source_second_item_no_advance(self):
        ...
        # 第二条：同源 sany，2 天后
        id2 = _seed_intel("sany", "三一", "新品发布", t + timedelta(days=2))
        _cluster_one(id2)
...
class TestAC4OutsideTimeWindowNewEvent:
    def test_beyond_7_days_creates_new_event(self):
        ...
        id1 = _seed_intel("sany", "三一", "新品发布", t1)
        event_id1 = _cluster_one(id1)

        # 10 天后——超出 ±7 天窗
        id2 = _seed_intel("ccma", "三一", "新品发布", t1 + timedelta(days=10))
        event_id2 = _cluster_one(id2)
...
class TestAC5DifferentEventTypeNoMerge:
    def test_different_event_type_creates_separate_events(self):
        ...
        id2 = _seed_intel("ccma", "三一", "专利公开", t + timedelta(days=1))
        event_id2 = _cluster_one(id2)
...
class TestAliasNormalization:
    def test_alias_subject_merges_into_same_event(self):
        ...
        id2 = _seed_intel("ccma", "三一重工", "新品发布", t + timedelta(days=2))
        event_id2 = _cluster_one(id2)
...
class TestBackfillCommand:
    def test_cluster_backfill_attaches_existing_extracted(self, capsys):
        ...
        t = datetime(2026, 8, 27, 10, 0, 0)
        _seed_intel("sany", "三一", "新品发布", t)
        _seed_intel("ccma", "三一", "新品发布", t + timedelta(days=1))  # 第二独立信源
        _seed_intel("sany", "徐工", "财报", t + timedelta(days=2))
