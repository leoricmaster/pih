...
继承 spike `decode_body` 的三级链路并修复两处遗留 Minor：
1. meta charset 扫描窗口 2048→4096（深埋 meta 不再漏）；
2. 增加 HTML 实体解码（&amp; &#xx; 等，SPK-1 遗留 Minor「HTML 实体未解码」）。

链路：HTTP 头 charset → HTML meta charset → utf-8/gbk 严格试探 → utf-8/replace。
关键修正（SPK-1 报告纠错）：cehome 非 GBK，是 HTTP 头缺 charset 导致默认 ISO-8859-1
...
# meta charset 扫描窗口（Sprint 0 spike 用 2048，深埋 meta 会漏 → 扩到 4096）
...
def decode_entities(text: str) -> str:
    ...
    SPK-1 遗留 Minor「HTML 实体未解码」的契约落地。
    """
