...
**方案 A：Dify 聊天界面 + 飞书多维表格** — 优点：零前端开发；缺点：交付不了详情页 AC；Agent 只能解析页面，脆弱且不可持续

**方案 B：完整自研 Web（前后端分离）+ 独立 API 服务** — 优点：体验上限高；缺点：单用户下过度设计；两套查询逻辑必然漂移；1 人运维多一个组件

**方案 C：单一 FastAPI 应用双出口——服务端模板 Web + 只读 JSON API** — 优点：约 3–5 人日；过滤/排序/引用拼装共用一套逻辑，Web 与 API 结果天然一致；Agent 接入零额外开发；缺点：Web 交互上限受服务端模板约束（只读场景无此需求）
...
2. 消费侧故事（TASK-2.01.01 / TASK-2.02.01）的全部 AC 可交付；
3. 1 人运维约束下不新增任何组件。
