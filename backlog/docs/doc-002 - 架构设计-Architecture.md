...
- 版本：V2.0 · **Day0 重写**——以立项文档 doc-001 V2.0 与需求树（backlog/tasks，TASK-1..4）为唯一事实源，按 **C4 模型**重建；本文只写**目标态**，不陈述实现进度（进度由代码与测试反映）
- 配套：doc-001 立项 · doc-003 NFR · ADR backlog/decisions（§10 索引）· 可点击原型 docs/prototype.html（IA 验收参照，非架构事实源）
...
**四条贯穿性约束**（所有模块设计不得违反，NFR doc-003 依赖本节与 §8）：
...
- **结论资产化**：终态假设即**结论**——判断层知识资产，含依据、证据链与结论时间；可检索、可引用进规划文档、可挂为新假设的证据（认知迭代）；**不回写情报表**（事实/判断两层分治，doc-001 §3）；
