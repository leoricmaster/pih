...
> 交付：2026-08-25，3 轮提示词迭代（v1/v2/v3），25 样本 MiniMax-M3 实测。枚举命中率 52–56%，字段准确率 25–31%（瓶颈为自由文本严格相等评分口径）。

#### SPK-3（已交付）LangGraph + 自有模型服务连通性验证

> 端到端跑通一次抽取。决策语境：ADR-004 后果节。
> 交付：2026-08-25，LangGraph 1.2.11 三节点图（粗筛→抽取→校验）跑通 25 样本，端到端成功率 92%（23/25 产出完整 schema），API 调用成功率 100%。粗筛 kept 92%（2 条假阴性，锂矿/期货口径偏窄），validate 重问率 22%。决策结论：ADR-004 维持。
