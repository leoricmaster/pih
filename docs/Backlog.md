...
- 版本：V3.0（重立需求树——只区分梳理过的需求与未分解的未来需求，不携带排期信息：「互联网情报闭环」单 Epic 垂直切片承载（生产与消费一体、不拆开验收），人工信源纳入为独立 Epic，Agent 消费等收进未来简表。配合《Product Requirements.md》、《Architecture.md》）
...
（初期各角色由同一人承担，见需求文档 §2；Agent 承担消费者 / 贡献者为其自然扩展，对应未分解的未来需求——Agent 同源消费与录入网关。）

---

## EPIC-1 互联网情报闭环

**用户目标**：作为产品规划者，系统替我盯住注册的信源——自动采集、存档原文、结构化出带可信度的情报；我在 Web 上消费它——筛选、溯源、判断可信度、顺手纠错；我的反馈驱动抽取与粗筛迭代，系统越用越准。**生产与消费是这个闭环的两面，不拆开验收。**
...
- **终态人工**：`多源确认` / `已证伪` 两个终态必须人工触发，自动跃迁全程写日志（操作者=system）；
...
| 方向 | 一句话 | 对应 PRD |
|---|---|---|
| Agent 同源消费 | 只读 JSON API 按与 Web 相同条件组合查询情报（同源同序、凭据鉴权），供 Agent 程序化消费而非解析网页 | §4.5 |
| 自然语言问答 | 带引用的 RAG 问答 + 多轮追问，答案强制附情报 ID / 来源 / 置信度，无可靠情报时如实回答不编造 | §4.5 |
| 竞品功能 / 参数矩阵 | 竞品 × 功能项对比表（有 / 无 / 规划中）+ 关键性能参数表，单元格关联支撑情报 | §4.4 |
| 竞品档案页 | 每个竞品主体一页：基本信息 + 情报聚合 | §4.4 |
| 周报 / 月报 | 定期简报（动态分组 + 趋势归纳 + 弱信号），无新情报仍推送不静默 | §4.6 |
| 重大事件即时推送 | L1 信源重大事件（新品 / 中标 / 人事）完成入库后 30 分钟内推送，同事件多源不重复 | §4.6 |
| 专题调研 | 发起专题任务（如"3D 引导系统竞品对比"），智能体限时深挖产出报告 | §4.8 |
| 信源画像页 | 基于核实历史重估来源可靠性（被证伪率 / 首发率 / 覆盖领域） | §4.1 |
| 过期时效治理 | 超有效期情报自动降权 + 复核提醒，列表显式标识并排序靠后 | §4.3 |
